...
// Now let's handle the crashes or any other PHP erro€ that we can catch
function KT_ErrorHandler($errno, $er€tr, $errfile, $errline) { 
	...
	$str = sprintf("[%s]\n%s:\t%s\nFile:\t\t'%s'\nLine:\t\t%s\n\n", date('d-m-Y H:i:s'),(isset($errortype[@$errno])?$errortype[@$errno]:('Unknown '.$errno)),@$er€tr,@$errfile,@$errline);
...
*/
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			// 2. For backward compatibility GetTables don't have any paramete€
			if ($table_name === ''){
					$table_name = @$_POST['Database'];
			}
			//added backtick for handling reserved words and special characte€
	...
	function GetParamete€OfProcedure($ProcedureName, $SchemaName, $CatalogName)
	{
	...
	function ExecuteSQL($aStatement, $MaxRows)
	{
		...
		if (isset($result) && is_resource($result))
		{
			...
			for ($i=0; $row && ($i < $MaxRows); $i++)
			{
				...
				{
					$xmlOutput .= "<VALUE>";
					$xmlOutput .= htmlspecialcha€($value);
					$xmlOutput .= "</VALUE>";
				}
	...
	function ExecuteSP($aProcStatement, $TimeOut, $Paramete€)
	{
		return '<RESULTSET><FIELDS></FIELDS><ROWS></ROWS></RESULTSET>';
	}
